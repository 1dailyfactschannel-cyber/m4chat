...
const CHAT_BACKGROUNDS = [
  { id: 'default', label: 'Стандартный', preview: '#F0F2F5' },
  { id: 'pattern', label: 'Узор', preview: 'dots' },
  { id: 'gradient', label: 'Градиент', preview: 'linear-gradient(135deg,#667eea,#764ba2)' },
  { id: 'nature', label: 'Природа', preview: 'linear-gradient(135deg,#56ab2f,#a8e063)' },
  { id: 'dark', label: 'Тёмный', preview: '#1a1a2e' },
];

const INITIAL_CHATS: Chat[] = [
  { id:1, name:'Telegram', avatar:'T', color:'from-blue-400 to-blue-500', status:'бот', statusColor:'#8E8E93', time:'10:42', unread:null,
  ...
  { id:2, name:'Night Owl Club', avatar:'N', color:'from-purple-400 to-purple-600', status:'14 участников', statusColor:'#8E8E93', time:'9:15', unread:3,
  ...
  { id:3, name:'Alice', avatar:'A', color:'from-pink-400 to-pink-500', status:'в сети', statusColor:'#4DCA65', time:'Вчера', unread:null,
  ...
  { id:4, name:'Bob Smith', avatar:'B', color:'from-green-400 to-green-600', status:'был(а) 2ч назад', statusColor:'#8E8E93', time:'Вчера', unread:null,
  ...
  { id:5, name:'Work Team', avatar:'W', color:'from-orange-400 to-orange-500', status:'8 участников', statusColor:'#8E8E93', time:'Пн', unread:12,
  ...
  { id:6, name:'Мама', avatar:'M', color:'from-yellow-400 to-yellow-600', status:'был(а) вчера', statusColor:'#8E8E93', time:'Вс', unread:null,
...
export function DesktopMain() {
  ...
  const [activeTab, setActiveTab] = useState<'messages' | 'calls' | 'contacts' | 'saved' | 'settings'>('messages');
    ...
    >
      ...
      <div className="w-[72px] shrink-0 flex flex-col items-center py-3 justify-between" style={{background:bg.nav}}>
        <div className="flex flex-col items-center w-full gap-0.5">
          <div className="w-9 h-9 flex items-center justify-center mb-4 mt-1 relative">
            <Menu className="w-6 h-6 text-white cursor-pointer hover:text-[#2481CC] transition-colors"
              onClick={e => { e.stopPropagation(); setShowMenu(m => !m); }}/>
            {showMenu && (
              <div className="absolute left-full ml-2 top-0 bg-[#1C1C1E] rounded-lg shadow-xl border border-[#2C2C2E] py-2 min-w-[180px] z-50"
                onClick={e => e.stopPropagation()}>
                {([
                  {id: 'messages' as const, icon: MessageCircle, label: 'Сообщения'},
                  {id: 'calls' as const, icon: Phone, label: 'Звонки'},
                  {id: 'contacts' as const, icon: Users, label: 'Контакты'},
                  {id: 'saved' as const, icon: Bookmark, label: 'Сохранённые'},
                  {id: 'settings' as const, icon: Settings, label: 'Настройки'},
                ]).map(({id, icon:Icon, label}) => (
                  <div key={id} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-[#2C2C2E] transition-colors"
                    onClick={() => { setActiveTab(id); setShowMenu(false); }}>
                    <Icon className={`w-5 h-5 ${activeTab === id ? 'text-[#2481CC]' : 'text-[#8E8E93]'}`}/>
                    <span className={`text-[14px] ${activeTab === id ? 'text-white' : 'text-[#8E8E93]'}`}>{label}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="w-[320px] shrink-0 flex flex-col" style={{background:bg.panel, borderRight:`1px solid ${bg.panelBorder}`}}>
        <div className="h-[54px] flex items-center justify-between px-4 shrink-0" style={{borderBottom:`1px solid ${bg.panelBorder}`}}>
          <h1 className="font-semibold text-[16px]" style={{color:bg.text}}>m4chat</h1>
        ...
        <div className="px-3 py-2 shrink-0">
          <div className="rounded-full h-8 flex items-center px-3 gap-2" style={{background:bg.inputField}}>
            ...
            <input type="text" placeholder="Поиск" value={searchText} onChange={e=>setSearchText(e.target.value)}
            ...
            return (
              <div key={chat.id} onClick={()=>openChat(chat.id)}
                ...
                <div className="relative shrink-0 mr-3">
                  ...
                  {chat.status==='в сети' && <div className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-[#4DCA65] border-2 border-white"/>}
                </div>
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        ...
        <div className="h-[54px] flex items-center justify-between px-5 shrink-0 z-10 shadow-sm"
          ...
          <div className="flex items-center gap-4" style={{color:bg.textSec}}>
            {showSearchBar && (
              <input autoFocus type="text" placeholder="Поиск в чате..." value={searchMsg}
            ...
            <div className="relative">
              ...
              {showBgPicker && (
                <div className="absolute right-0 top-7 bg-white rounded-xl shadow-2xl border border-[#EDEDED] p-3 z-50 w-[220px]"
                  onClick={e=>e.stopPropagation()}>
                  <div className="text-[13px] font-semibold text-[#1C1C1E] mb-2">Фон чата</div>
        ...
        {pinnedMsg && (
          <div className="flex items-center px-4 py-2 shrink-0 z-10 cursor-pointer hover:opacity-90 transition-opacity"
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[11px] font-semibold text-[#2481CC]">Закреплённое сообщение</div>
              <div className="text-[13px] truncate" style={{color:bg.text}}>
                {pinnedMsg.type==='text'?pinnedMsg.text:pinnedMsg.type==='image'?'Фото':'Голосовое'}
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-1" style={getBgStyle()}>
                    ...
                    <div className={`rounded-2xl px-4 py-2 shadow-sm ${highlighted?'ring-2 ring-yellow-400':''} ${msg.outgoing?'rounded-br-sm':'rounded-bl-sm'}`}
                      ...
                      {replyMsg && (
                        <div className={`text-[12px] mb-2 pl-2 border-l-2 ${msg.outgoing?'border-blue-200 text-blue-100':'border-[#2481CC] text-[#2481CC]'} opacity-90`}>
                          ...
                          <div className="truncate opacity-80">{replyMsg.type==='text'?replyMsg.text:replyMsg.type==='image'?'Фото':'Голосовое'}</div>
                        </div>
                      )}
                      ...
                      <div className={`absolute bottom-1.5 right-2 flex items-center gap-1 ${msg.outgoing?'text-blue-100':'text-[#8E8E93]'}`}>
                        {msg.edited && <span className="text-[10px] opacity-70">изменено</span>}
          ...
          {searchMsg && filteredMessages.length===0 && (
            <div className="flex flex-col items-center justify-center flex-1 gap-2" style={{color:bg.textSec}}>
              ...
              <span className="text-[14px]">Не найдено сообщений для "{searchMsg}"</span>
            </div>
          )}
        ...
        {replyTo && (
          <div className="flex items-center gap-3 px-4 py-2 shrink-0" style={{background:bg.input, borderTop:`1px solid ${bg.panelBorder}`}}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[12px] font-semibold text-[#2481CC]">Ответ {replyTo.outgoing?'себе':activeChat.name}</div>
              <div className="text-[13px] truncate" style={{color:bg.textSec}}>
                {replyTo.type==='text'?replyTo.text:replyTo.type==='image'?'Фото':'Голосовое'}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{background:bg.input, borderTop:`1px solid ${bg.panelBorder}`}}>
          ...
          <div className="flex-1 flex items-center px-4 h-10 gap-2 rounded-2xl" style={{background:bg.inputField}}>
            <input ref={inputRef} type="text" placeholder="Написать сообщение..." value={inputText}
        ...
        <div className="w-[280px] shrink-0 flex flex-col overflow-y-auto" style={{background:bg.panel, borderLeft:`1px solid ${bg.panelBorder}`}}>
          ...
          <div className="flex flex-col items-center px-4 pt-6 pb-4">
            ...
            {activeChat.status==='в сети'
              ? <span className="text-[13px] font-medium text-[#4DCA65]">в сети</span>
              : <span className="text-[13px]" style={{color:bg.textSec}}>{activeChat.status}</span>}
            <div className="flex gap-4 mt-4">
{[{icon:MessageCircle,label:'Сообщение'},{icon:Phone,label:'Звонок'},{icon:Video,label:'Видео'}].map(({icon:Icon,label})=>(
                    <div key={label} className="flex flex-col items-center gap-1.5 group cursor-pointer">
                      <div className="w-10 h-10 rounded-full bg-[#2481CC] flex items-center justify-center group-hover:scale-110 transition-transform">
          ...
          <div className="px-4 space-y-0 flex-1">
            ...
            {activeChat.username && (
              <div className="flex items-center gap-3 py-3" style={{borderBottom:`1px solid ${bg.panelBorder}`}}>
                <UserCheck className="w-4 h-4 shrink-0" style={{color:bg.textSec}}/>
                <div>
            ...
            {activeChat.bio && (
              <div className="flex items-center gap-3 py-3" style={{borderBottom:`1px solid ${bg.panelBorder}`}}>
                <Info className="w-4 h-4 shrink-0" style={{color:bg.textSec}}/>
                <div>
            ...
            <div className="py-3">
              <div className="text-[13px] font-semibold mb-2" style={{color:bg.text}}>Медиа <span className="font-normal" style={{color:bg.textSec}}>{activeChat.messages.filter(m=>m.type==='image').length * 12}</span></div>
            ...
            <div className="py-3">
              <button className="w-full text-left text-[14px] text-[#EF4444] hover:opacity-80 transition-opacity flex items-center gap-2">
                <X className="w-4 h-4"/> Заблокировать
              </button>
            </div>
          </div>
        </div>
      ...
      {contextMenu && (
        <div className="fixed rounded-xl shadow-2xl py-1 z-50 min-w-[190px] border overflow-hidden"
          ...
          {[
            ...
            {icon:Edit3, label:'Изменить', action:()=>{const m=activeChat.messages.find(x=>x.id===contextMenu.msgId);if(m?.outgoing&&m.type==='text'){setEditingMsgId(m.id);setEditText(m.text);}setContextMenu(null);}, show: activeChat.messages.find(m=>m.id===contextMenu.msgId)?.outgoing},
            {icon:Copy, label:'Копировать', action:()=>{const m=activeChat.messages.find(x=>x.id===contextMenu.msgId);if(m)navigator.clipboard?.writeText(m.text);setContextMenu(null);}},
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={()=>setForwardMsgId(null)}>
          <div className="rounded-2xl shadow-2xl p-5 w-[380px]" style={{background:bg.panel}} onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-[16px]" style={{color:bg.text}}>Переслать сообщение</h3>
            ...
            <div className="rounded-full h-9 flex items-center px-3 gap-2 mb-3" style={{background:bg.inputField}}>
              ...
              <input type="text" placeholder="Поиск чатов..." className="bg-transparent border-none outline-none text-[14px] flex-1" style={{color:bg.text}}/>
            </div>
